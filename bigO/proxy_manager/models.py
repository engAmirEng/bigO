...
class Config(TimeStampedModel, SingletonModel):
    ...
    xray_config_template = models.TextField(null=True, blank=False, help_text="{{ node, inbound_parts, rule_parts, balancer_parts }}")
...
class NodeOutbound(TimeStampedModel, models.Model):
    ...
    class Meta:
        constraints = [
            UniqueConstraint(fields=("name", "node"), name="unique_name_node")
        ]
...
class ConnectionRule(TimeStampedModel, models.Model):
    ...
    destination_region = models.ForeignKey(Region, on_delete=models.CASCADE, related_name="destinationregion_connectionrules")
...
class Subscription(TimeStampedModel, models.Model):
    connection_rule = models.ForeignKey(ConnectionRule, on_delete=models.PROTECT, related_name="connectionrule_subscriptions", null=True, blank=False)
