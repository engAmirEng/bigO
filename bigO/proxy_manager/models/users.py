...
from bigO.proxy_manager.subscription import AVAILABLE_SUBSCRIPTION_PLAN_PROVIDERS, AVAILABLE_SUBSCRIPTION_PLAN_PRICE_PROVIDERS
from bigO.proxy_manager.subscription.base import BaseSubscriptionPlanProvider, BaseSubscriptionPlanPriceProvider
...
class PlanPriceProviderSubscriberMixin(models.Model):
    ...
    def plan_provider_price_cls(self) -> type[BaseSubscriptionPlanPriceProvider]:
        return [i for i in AVAILABLE_SUBSCRIPTION_PLAN_PRICE_PROVIDERS if i.TYPE_IDENTIFIER == self.price_provider_key][0]
...
class AgencyUserGroup(TimeStampedModel, models.Model):
    ...
    class Meta:
        ordering = ["-created_at"]
        constraints = [
            UniqueConstraint(fields=("name", "agency"), name="unique_name_agency__agencyusergroup")
        ]
...
class AgencyUserGroupPlanSpec(TimeStampedModel, PlanPriceProviderSubscriberMixin, models.Model):
    ...
    class Meta:
        constraints = [UniqueConstraint(fields=("agencyusergroup", "plan"), name="unique_agencyusergroup_plan_agencyusergroupplanspec")]
