...
def get_xray_outbounds(node_obj):
    ...
    """ % (tag, ", ".join([f'"{i}"' for i in selectors])) for tag, selectors in balancers.items()])
#     # freedom
#     freedom_conf = """
# {
#   "tag": "freedom",
...
# """
#
#     res.append({"tag": "freedom", "conf": freedom_conf})
...
def get_xray_conf(node_obj) -> tuple[str, str, dict]:
    ...
    for inbound in models.Inbound.objects.filter(is_active=True, is_template=False):
        xray_inbound = django.template.Template(
            "{% load node_manager %}" + inbound.inbound_template
        ).render(context=django.template.Context({"node": node_obj}))
        if inbound_parts:
            inbound_parts += ",\n"
    ...
    for connection_rule in models.ConnectionRule.objects.filter():
        ...
        for inbound in models.Inbound.objects.filter(is_active=True, is_template=True):
            ...
            ).render(context=django.template.Context({"node": node_obj, "inbound_tag": inbound_tag}))
            inbound_tags.append(inbound_tag)
            if inbound_parts:
                inbound_parts += ",\n"
            inbound_parts += xray_inbound
        xray_rules = django.template.Template("{% load node_manager proxy_manager %}" + connection_rule.xray_rules_template).render(
