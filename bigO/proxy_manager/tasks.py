...
@app.task
def subscription_nearly_ended_notify(remained_seconds: int, remained_bytes: int):
    base_time = timezone.now() + timedelta(seconds=remained_seconds)
    near_end_periods_qs = (
        models.SubscriptionPeriod.objects.filter(selected_as_current=True)
        .ann_limit_passed_type(base_bytes=remained_bytes, base_time=base_time)
        .filter(limit_passed_type__isnull=False)
    )
