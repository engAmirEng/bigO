...
class OwnerBotFilter(Filter):
    async def __call__(
        self, update: Union[Message, CallbackQuery], user: TelegramUser, bot_obj: telegram_bot_models.TelegramBot, **kwargs
    ) -> bool:
...
def query_magic_dispatcher(pathname: QueryPathName) -> aiogram.MagicFilter:
    ...
    """
    if pathname == QueryPathName.ASSOCIATE_TO_ACCOUNT:
