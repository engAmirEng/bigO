...
def get_subscription_profile_startlink(
    bot_obj: models.Panel, subscription_profile: proxy_manager_models.SubscriptionProfile, transfer_ownership: bool = False
) -> str:
    ...
    link = get_dispatch_query(
        bot_username=bot_obj.tusername, pathname=QueryPathName.ASSOCIATE_TO_ACCOUNT, key=key
    )
    return link
