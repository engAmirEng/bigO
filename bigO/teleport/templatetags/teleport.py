...
@register.simple_tag(takes_context=False)
def admin_profile_detail_link(bot_username: str, subscription_profile: proxy_manager_models.SubscriptionProfile):
    link = get_dispatch_query(
        bot_username=bot_username, pathname=QueryPathName.ADMIN_PROFILE_DETAIL, id=subscription_profile.id
    )
    return link
